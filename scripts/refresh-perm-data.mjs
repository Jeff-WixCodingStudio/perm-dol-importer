...
const payload = {
  ...
  sourceRelease: workbookUrl.split("/").pop(),
  chartSeries: {
    Certified: buildChartSeries(certified),
    Processed: buildChartSeries(records)
  }
};
...
function newestDisclosureUrl(html) {
  const urls = [...String(html).matchAll(/href=["']([^"']+\.xlsx(?:\?[^"']*)?)["']/gi)]
    .map((match) => new URL(match[1], PERFORMANCE_PAGE).href)
    .filter((url) => /PERM/i.test(url) && !/record[ _-]?layout/i.test(url));
  return [...new Set(urls)].sort((left, right) => fiscalYear(right) - fiscalYear(left))[0];
}

function fiscalYear(url) {
...
function buildChartSeries(records) {
  ...
  return {
    daily: dailyDates.map((date) => dailyCounts[dayKey(date)] || 0),
    dailyLabels: dailyDates.map(formatShortDate),
    weekly: [1, 2, 3, 4, 5, 6, 0].map((day) => records.filter((record) => record.decisionDate.getDay() === day).length),
    weeklyLabels: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"],
    weeklyVolume: Array.from({ length: 4 }, (_, index) => weeklyCounts[weekKey(addDays(latest, -21 + index * 7))] || 0),
    weeklyVolumeLabels: Array.from({ length: 4 }, (_, index) => formatShortDate(addDays(latest, -21 + index * 7))),
    monthlyVolume: monthDates.map((date) => monthlyCounts[monthKey(date)] || 0),
    monthlyVolumeLabels: monthDates.map((date) => date.toLocaleDateString("en-US", { month: "short", year: "numeric" })),
    activity: "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((letter) => letters[letter] || 0),
    activeLetters: "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((letter) => records.filter((record) => monthKey(record.decisionDate) === latestMonth && employerLetter(record.employer) === letter).length),
